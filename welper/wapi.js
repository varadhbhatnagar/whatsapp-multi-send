...
window.WAPI.ReplyMessage = function (idMessage, message, done) {
    ...
    if (messageObject === undefined) {
        if (done !== undefined) done(false);
        window.WAPI._replyToMsgIDStatus = false;
        return false;
    }
            ...
            chat.sendMessage(message, {quotedMsg: messageObject}, messageObject).then(function () {
                ...
                function check() {
                    for (let i = chat.msgs.models.length - 1; i >= 0; i--) {
                        ...
                        window.WAPI._replyToMsgIDStatus = true;
                        return True;
                    }
                    trials += 1;
                    if (trials > 30) {
...
window.WAPI.sendMessageToIDUnknown = function (id, message, done) {
    try {
    // Create user
...
window.WAPI.getUnreadMessages = function (includeMe, includeNotifications, use_unread_count, done) {
    ...
    for (let chat in chats) {
        ...
        if (messageGroup.messages.length > 0) {
            ...
            window.WAPI._missedMessagesBuffer.push(messageGroup);
...
window.WAPI._missedMessagesBuffer = [];
